...
```
一、算法正确性分析
（根据你的代码和测试数据，分析算法是否正确、边界条件处理等）

二、时间复杂度分析
（根据你写的代码，推导具体的时间复杂度，如 O(n log n)）

三、空间复杂度分析
（分析辅助空间和递归栈消耗等）

四、实验心得
（结合实验中遇到的具体问题、调试过程来写，不要套话）
```

脚本会优先读取 `analysis.txt` 填入报告。如果没有这个文件，会回退到模板生成的分析。
