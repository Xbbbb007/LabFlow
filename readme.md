...
自动生成河南理工大学《算法分析与设计》课程实验报告。

基于 Word 模板填充，封面页和评分页保持原样，脚本负责生成各实验内容页。

> **💡 智能环境检测**：当您告诉 AI Agent 要使用的编程语言后，Agent 会自动检测您的环境配置情况。如果发现问题（如版本过低、路径混乱、缺少编译器等），Agent 会主动询问是否需要帮忙配置，无需手动操作。
>
> **🎯 智能语言识别**：您可以在请求中直接指定语言，例如说"帮我写 Java 实验报告"，Agent 会自动识别并跳过语言询问，直接进入环境检测。如果未指定语言，Agent 会主动询问您使用哪种语言。
...
脚本会自动检测模板中是否已填写内容，并根据情况处理：

| 区域           | 检测逻辑                     | 处理方式                             |
| -------------- | ---------------------------- | ------------------------------------ |
| 实验题目       | 检查"实验题目："后是否有内容 | 未填写 → 从指导书提取；已填写 → 保留 |
| 实验目的和要求 | 检查"实验目的："后是否有内容 | 未填写 → 从指导书提取；已填写 → 保留 |
| 实验过程       | 始终清空后重新填充           | 从 src/ 目录读取代码写入             |
| 实验结果       | 始终清空后重新填充           | 从 output/ 目录读取截图插入          |
| 实验分析       | 始终清空后重新填充           | 自动生成分析内容                     |
| 实验成绩       | 不修改                       | 留空给教师填写                       |

**注意**：如果用户已经在模板中填写了实验题目和实验目的，脚本会保留用户填写的内容，不会覆盖。
