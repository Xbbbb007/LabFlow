...
自动生成各类**程序设计类课程**的实验报告。

基于 Word 模板填充，封面页和评分页保持原样，脚本负责生成各实验内容页。

## 适用课程

本工具适用于以下类型的程序设计类实验课程：

| 课程类型       | 示例课程                                       |
| -------------- | ---------------------------------------------- |
| **算法类**     | 算法设计与分析、数据结构与算法、算法导论       |
| **编程语言类** | Python 程序设计、Java 程序设计、C 语言程序设计 |
| **数据结构类** | 数据结构、高级数据结构                         |
| **软件开发类** | 面向对象程序设计、软件工程、Web 开发           |
| **系统类**     | 操作系统、计算机网络、数据库系统               |

**支持的编程语言**：Python、Java、C、C++、JavaScript、Go、Rust 等
...
├── output/                 # 运行结果
│   ├── exp1/               # 实验一结果
│   │   ├── run_result.png  # 运行截图（必须）
│   │   ├── analysis.txt    # Agent 写的实验分析（推荐）
│   │   └── output.txt      # 文本输出（可选）
│   └── ...
├── report/                 # 生成的报告（输出目录）
...
| 实验分析       | analysis.txt 或自动生成 | Agent 写分析文本，脚本优先读取；无文件时回退到内置模板（自动适配课程类型） |
| 实验成绩       | 留空         | 教师填写                          |
...
| 方式 | 文件 | 说明 |
| --- | --- | --- |
| **Agent 写分析（推荐）** | `output/exp{N}/analysis.txt` | 由 AI Agent 根据实际代码和运行结果撰写，内容更有针对性 |
| **脚本内置模板** | 自动生成 | 脚本根据课程类型（OOP/算法）自动选择分析模板 |

**课程类型自动识别**：脚本会根据实验名称和代码特征判断属于程序设计/OOP课程还是算法课程：
- **OOP/程序设计课程**（如 Java、Python 基础）→ 侧重知识点运用、面向对象概念、调试心得
- **算法课程**（如递归、动态规划）→ 侧重正确性分析、时间/空间复杂度

**analysis.txt 格式**（3段，共150-200字）：
